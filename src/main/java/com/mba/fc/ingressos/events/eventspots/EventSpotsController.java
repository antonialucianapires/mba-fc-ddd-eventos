...
public class EventSpotsController {
    ...
    @GetMapping
    public Set<EventSpotResponse> getSpots(@PathVariable String eventId, @PathVariable String sectionId) {
        EventSectionId id = new EventSectionId(sectionId);
        EventSection section = eventService.listSections(new EventId(eventId)).stream()
